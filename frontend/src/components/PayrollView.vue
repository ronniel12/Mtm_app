      ...
      <table class="payroll-table">
        ...
        <tbody>
          ...
          <tr v-for="(trip, index) in filteredEmployeeTrips" :key="trip.id" class="data-row" :class="{ 'alt-row': index % 2 === 1 }">
            ...
            <td class="plate-cell">{{ trip.truck_plate }}</td>
            <td class="invoice-cell">{{ trip.invoice_number }}</td>
            <td class="destination-cell">{{ trip.full_destination }}</td>
            <td class="bags-cell text-center">{{ trip.number_of_bags }}</td>
            ...
            <td class="total-cell text-right">{{ trip._rate && trip.number_of_bags ? formatCurrency((trip._rate - 4) * trip.number_of_bags * trip._commission) : '0.00' }}</td>
          </tr>
...
const expandEmployeeTrips = (trips) => {
  ...
  trips.forEach(trip => {
    ...
    if (trip.driver === selectedEmployeeUuid.value) {
      expandedTrips.push({
        ...trip,
        // Map database snake_case fields to snake_case for component use
        invoice_number: trip.invoice_number,
        full_destination: trip.full_destination || trip.destination,
...
const totalBags = computed(() => {
  return filteredEmployeeTrips.value.reduce((sum, trip) => {
    return sum + (trip.number_of_bags || 0)
  }, 0)
})
...
<td style="border: 1px solid #000; padding: 2px; text-align: center; font-size: 8px;">${trip.number_of_bags}</td>
...
<td style="border: 1px solid #000; padding: 2px; text-align: right; font-size: 8px;">₱${trip._total ? formatCurrency((trip._rate - 4) * trip.number_of_bags * trip._commission) : '0.00'}</td>
</tr>`
  })
...
const exportExcel = () => {
  ...
  const data = [
    ...
    ...filteredEmployeeTrips.value.map(trip => [
      ...
      `₱${trip._rate && trip.number_of_bags ? formatCurrency((trip._rate - 4) * trip.number_of_bags * trip._commission) : '0.00'}`
    ]),
...
<td style="border: 1px solid #000; padding: 6px; text-align: right;">₱${trip._rate && trip.number_of_bags ? formatCurrency((trip._rate - 4) * trip.number_of_bags * trip._commission) : '0.00'}</td>
</tr>`
  })
...
const savePayslip = async () => {
  ...
  const payslipData = {
    ...
    trips: filteredEmployeeTrips.value.map(trip => ({
      ...
      rate: trip._rate,
      total: trip._rate && trip.number_of_bags ? (trip._rate - 4) * trip.number_of_bags * trip._commission : 0 // Use same calculation as display
    })),
    totals: {
