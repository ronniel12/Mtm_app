          ...
          >
            ...
            <td class="destination-cell">
              <div class="route-cell">
                <span class="route-text">{{ trip.full_destination || buildFullAddress(trip) }}</span>
              </div>
            </td>
            <td class="bags-cell">{{ trip.number_of_bags || 0 }}</td>
            <td class="rate-cell">
            ...
            <td class="driver-cell">{{ getEmployeeNameByUuid(trip.driver) }}</td>
            <td class="helper-cell">{{ trip.helper ? getEmployeeNameByUuid(trip.helper) : 'Not assigned' }}</td>
...
const fetchData = async () => {
  ...
  try {
    const [tripsResponse, employeesResponse] = await Promise.all([
      axios.get(`${API_BASE_URL}/trips?page=${currentPage.value}&limit=${pageSize.value}`),
      axios.get(`${API_BASE_URL}/employees`)
    ])

    // Data is already transformed by the API, just use it directly
...
const calculateRatesForTrips = async () => {
  try {
    ...
    trips.value.forEach(trip => {
      ...
      if (destinationParts.length === 2) {
        ...
        if (exactMatch) {
          foundRate = exactMatch.newRates || exactMatch.rates
        }
      }

      // No fallback rates - if no exact match found, keep rate as null
      // This ensures we don't bill incorrect rates
    ...
    console.error('Error calculating trip rates:', error)
    // Mark all trips as having rate calculation errors
    trips.value.forEach(trip => {
      trip._rate = null
      trip._rateFound = false
      trip._total = 0
      trip._rateStatus = 'Error calculating rate'
    })
  }
}
...
const buildFullAddress = (trip) => {
  ...
  if (trip.full_destination) return trip.full_destination

  // For existing trips, try to build from farm data and destination
  const farmData = getFarmData(trip.farm_name)
  if (farmData && farmData.name) {
    return `${farmData.name}, ${farmData.barangay}, ${farmData.town}, ${farmData.province}`
  }

  // For trips where farm_name is like "Town - Province" format but not a recognized farm,
  // try to construct a reasonable display
  if (trip.farm_name && trip.farm_name.includes(' - ')) {
    const parts = trip.farm_name.split(' - ')
    if (parts.length === 2) {
  ...
  // Fallback to simple destination or farm_name as-is
  return trip.destination || trip.farm_name || ''
}
