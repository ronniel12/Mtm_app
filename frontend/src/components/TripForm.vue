    ...
    <form @submit.prevent="submitForm" class="form-grid">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input
            id="number_of_bags"
            v-model.number="formData.number_of_bags"
            type="number"
            min="1"
            required
            class="form-input"
...
const submitForm = async () => {
  ...
  try {
    ...
    // Construct full_destination if not already set (for manual entries)
    if (!data.full_destination && data.farm_name && data.destination) {
      const destinationParts = data.destination.split(' - ')
      if (destinationParts.length === 2) {
        data.full_destination = `${data.farm_name}, ${destinationParts[0]}, ${destinationParts[1]}`
      }
    }
...
watch(() => props.editTrip, async (newTrip) => {
  if (newTrip) {
    ...
    // Populate form data
    Object.assign(formData, {
      date: newTrip.date ? newTrip.date.split('T')[0] : '', // Extract date part only for HTML date input
      truck_plate: newTrip.truck_plate || newTrip.truckPlate || 'NGU 9174',
      invoice_number: newTrip.invoice_number || newTrip.invoiceNumber || '',
      origin: newTrip.origin || 'Dampol 2nd A, Pulilan Bulacan',
      destination: newTrip.destination || '',
      full_destination: newTrip.full_destination || newTrip.fullDestination || '',
      rate_lookup_key: newTrip.rate_lookup_key || newTrip.rateLookupKey || '',
      driver: newTrip.driver || 'MTM Driver',
      helper: newTrip.helper || '',
      number_of_bags: newTrip.number_of_bags || newTrip.numberOfBags || 1,
      status: newTrip.status || 'Completed'
    })

    // Set farm name after a small delay to avoid autocomplete interference
    await new Promise(resolve => setTimeout(resolve, 10))
    formData.farm_name = newTrip.farm_name || newTrip.farmName || ''
...
// Watch for destination changes to auto-populate rate_lookup_key
watch(() => formData.destination, (newDestination) => {
  if (newDestination) {
    const parts = newDestination.split(' - ')
    if (parts.length === 2) {
      formData.rate_lookup_key = newDestination
    }
  }
})
...
const farmSuggestions = computed(() => {
  if (!formData.farm_name || !allFarms.value.length) return []

  const input = formData.farm_name.toLowerCase().trim()
  if (!input) return []
...
const onFarmInput = () => {
  ...
  if (!formData.farm_name.trim() || farmSuggestions.value.length === 0) {
...
const selectFarm = (farm) => {
  ...
  formData.farm_name = selectedFarm.name
  formData.destination = `${selectedFarm.town} - ${selectedFarm.province}`
  formData.full_destination = selectedFarm.fullAddress || `${selectedFarm.name}, ${selectedFarm.town}, ${selectedFarm.province}`
  formData.rate_lookup_key = `${selectedFarm.town} - ${selectedFarm.province}`
...
const selectSuggestion = (suggestion) => {
  ...
  formData.rate_lookup_key = suggestion.value
