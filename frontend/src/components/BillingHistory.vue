  ...
  <div class="billing-history">
    ...
    <div class="filters-section" v-if="billings.length > 0">
      <div class="filter-controls">
        ...
        <div class="stats-info">
          ...
          <span class="stat-item">
            Pending: <strong>{{ billings.filter(b => b.payment_status === 'pending').length }}</strong>
          </span>
          <span class="stat-item">
            Paid: <strong>{{ billings.filter(b => b.payment_status === 'paid').length }}</strong>
          </span>
        </div>
      </div>
    </div>
    ...
    <div v-if="billings.length === 0" class="no-billings">
      <div class="no-billings-content">
        ...
        <router-link to="/billing" class="btn btn-primary">Create Billing Statement</router-link>
      </div>
    </div>
    ...
    <div v-else class="billings-grid">
      <div
        ...
        :key="billing.id"
        class="billing-card"
        :class="{ 'paid': billing.payment_status === 'paid', 'pending': billing.payment_status === 'pending' }"
      >
        <div class="billing-header">
          <div class="billing-status" :class="billing.payment_status">
            {{ billing.payment_status === 'paid' ? '✓ Paid' : '⏰ Pending' }}
          </div>
          <div class="billing-meta">
            <small>Created: {{ formatDate(billing.created_date) }}</small>
          </div>
        </div>
        ...
        <div class="billing-details">
          <div class="billing-info-row">
            ...
            <span class="value">{{ billing.period.period_text }}</span>
          </div>

          <div class="billing-info-row">
            <span class="label">Billing Number:</span>
            <span class="value">{{ billing.billing_number }}</span>
          </div>
          ...
          <div class="billing-info-row">
            ...
            <span class="value">{{ billing.prepared_by }}</span>
          </div>
        ...
        <div class="billing-actions">
          ...
          <button @click="togglePaymentStatus(billing)" class="btn" :class="billing.payment_status === 'paid' ? 'btn-secondary' : 'btn-success'">
            {{ billing.payment_status === 'paid' ? 'Mark as Pending' : 'Mark as Paid' }}
          </button>
    ...
    <div v-if="selectedBilling" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h2>{{ selectedBilling.billing_number }}</h2>
        ...
        <div class="modal-body">
          <div class="billing-info-section">
            ...
            <div class="info-grid">
              <div class="info-item">
                <label>Status:</label>
                <span :class="selectedBilling.payment_status">{{ selectedBilling.payment_status === 'paid' ? '✓ Paid' : '⏰ Pending' }}</span>
              </div>
              <div class="info-item">
                <label>Period:</label>
                <span>{{ selectedBilling.period.period_text }}</span>
              </div>
              <div class="info-item">
                <label>Created:</label>
                <span>{{ formatDate(selectedBilling.created_date) }}</span>
              </div>
              <div class="info-item">
                <label>Prepared by:</label>
                <span>{{ selectedBilling.prepared_by }}</span>
              </div>
            </div>
          </div>
              ...
              <table class="trips-table">
                ...
                <tbody>
                  <tr v-for="trip in selectedBilling.trips" :key="trip.id">
                    ...
                    <td>{{ trip.truck_plate }}</td>
                    <td>{{ trip.invoice_number }}</td>
                    <td>{{ trip.destination }}</td>
                    <td class="text-center">{{ trip.number_of_bags }}</td>
        ...
        <div class="modal-footer">
          ...
          <button @click="togglePaymentStatus(selectedBilling)" class="btn" :class="selectedBilling.payment_status === 'paid' ? 'btn-secondary' : 'btn-success'">
            {{ selectedBilling.payment_status === 'paid' ? 'Mark as Pending' : 'Mark as Paid' }}
          </button>
...
const filterBillings = () => {
  ...
  if (statusFilter.value !== 'all') {
    filtered = filtered.filter(billing =>
      billing.payment_status === statusFilter.value
    )
  }
  ...
  if (searchQuery.value.trim()) {
    ...
    filtered = filtered.filter(billing =>
      ...
      billing.prepared_by.toLowerCase().includes(query)
    )
  }
...
const togglePaymentStatus = async (billing) => {
  const newStatus = billing.payment_status === 'paid' ? 'pending' : 'paid'
  ...
  try {
    ...
    if (response.status === 200) {
      // Update local data
      billing.payment_status = newStatus

      console.log('Updated billing status:', billing.id, newStatus)
      alert(`Billing ${billing.billing_number} marked as ${newStatus === 'paid' ? 'Paid' : 'Pending'}`)
      return
    }
  ...
  try {
    ...
    alert(`Billing ${billing.billing_number} marked as ${newStatus === 'paid' ? 'Paid' : 'Pending'} (stored locally)`)
  ...
  if (selectedBilling.value) {
    selectedBilling.value.payment_status = newStatus
  }
}

const deleteBilling = async (billing) => {
  const confirmed = confirm(`Are you sure you want to delete billing "${billing.billing_number}"? This action cannot be undone.`)
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      alert(`Billing "${billing.billing_number}" has been deleted successfully.`)
      return
    }
  ...
  try {
    ...
    if (index !== -1) {
      ...
      alert(`Billing "${billing.billing_number}" has been deleted successfully (local storage only).`)
    }
...
<td style="border: 1px solid #000; padding: 6px; text-align: center;">${trip.truck_plate}</td>
<td style="border: 1px solid #000; padding: 6px; text-align: center;">${trip.invoice_number}</td>
<td style="border: 1px solid #000; padding: 6px; text-align: center;">${trip.destination}</td>
<td style="border: 1px solid #000; padding: 6px; text-align: center;">${trip.number_of_bags}</td>
...
<strong>Billing Number:</strong> ${billing.billing_number}<br>
<strong>Period Covered:</strong> ${billing.period.period_text}<br>
<strong>Date Generated:</strong> ${formatDate(billing.created_date)}
</div>

${tableHTML}

<div style="margin-top: 30px; padding: 15px 0; font-size: 14px; font-weight: bold;">
<strong>Prepared by:</strong> ${billing.prepared_by}
</div>
</body>
</html>`
