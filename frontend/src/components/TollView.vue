      ...
      <div v-for="group in pendingConfirmations" :key="group.id" class="duplicate-alert">
        <div class="alert-content">
          ...
          <div class="group-summary">
            ...
            <div class="summary-row">
              <strong>Total Bags:</strong> {{ group.trips.reduce((sum, trip) => sum + (trip.number_of_bags || 0), 0) }}
            </div>
          </div>
          ...
          <div class="trip-details-container">
            ...
            <div class="trip-details-list">
              <div v-for="trip in group.trips" :key="trip.id" class="trip-detail-item">
                <span class="invoice">{{ trip.invoice_number }}</span>
                <span class="farm" :title="trip.farm_name">{{ trip.farm_name || 'N/A' }}</span>
                <span class="plate">{{ trip.truck_plate }}</span>
                <span class="bags">{{ trip.number_of_bags || 0 }}</span>
              </div>
            </div>
          </div>
...
const fetchTrips = async () => {
  ...
  try {
    ...
    trips.value = response.data.trips.map(trip => ({
      ...trip,
      truck_plate: trip.truck_plate || trip.truckPlate,
      invoice_number: trip.invoice_number || trip.invoiceNumber,
      number_of_bags: trip.number_of_bags || trip.numberOfBags,
      fullDestination: trip.full_destination || trip.fullDestination || trip.destination,
      computedToll: trip.computed_toll || trip.computedToll,
      roundtripToll: trip.roundtrip_toll || trip.roundtripToll,
      actualTollExpense: trip.actual_toll_expense || trip.actualTollExpense
    }))
    // Sort trips by date descending (newest first)
    trips.value = trips.value.sort((a, b) => new Date(b.date) - new Date(a.date))
...
const detectPotentialDuplicates = (trips) => {
  ...
  trips.forEach((trip, index) => {
    ...
    const hasSmallLoadInGroup = group.some(t => (t.number_of_bags || 0) < 250);
