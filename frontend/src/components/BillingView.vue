...
const fetchBillingData = async () => {
  try {
    const [tripsRes, ratesRes] = await Promise.all([
      axios.get(`${API_BASE_URL}/trips?limit=1000`), // Get more trips for billing
      axios.get(`${API_BASE_URL}/rates`)
    ])

    const tripsData = tripsRes.data.trips || tripsRes.data // Handle pagination response
    ...
    const mappedTrips = rawTrips.map(trip => ({
      ...
      invoiceNumber: trip.invoice_number,
      numberOfBags: trip.number_of_bags,
      fullDestination: trip.full_destination || trip.destination
    }))
